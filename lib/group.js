...
// tagpays.js - Plugin Anita-V4 pour taguer les membres par pays
// Auteur: Assistant AI | Compatible avec Anita-V4 by David Cyril Tech
...
module.exports = {
  name: 'tagpays',
  alias: ['tagp', 'tp', 'tagcountry'],
  description: 'Taguer les membres selon leur indicatif téléphonique par pays (Admin uniquement)',
  category: 'Admin',
  use: 'tagpays <pays>',
  owner: false,
  admin: true,
  ...
  async execute(anita, m, { participants, args, reply, prefix, command, isCreator, isAdmins, isBotAdmins }) {
    try {
      // Vérification groupe
      if (!m.isGroup) {
      ...
      // Vérification admin (compatible avec la structure Anita-V4)
      ...
      if (!args[0]) {
        return reply(`📍 *Utilisation:* ${prefix}${command} <pays>

*Exemples:*
• ${prefix}${command} BF
• ${prefix}${command} Burkina Faso  
• ${prefix}${command} France
• ${prefix}${command} CI
...
Tapez ${prefix}${command} pour voir plus d'exemples.`);
      }
      ...
      // Récupération des participants du groupe (compatible Anita-V4)
      const groupMetadata = await anita.groupMetadata(m.chat);
...
_Commande exécutée par un administrateur_`);
      ...
      // Message de tag
      const message = `${flagEmoji} *Membres de ${countryName}* (${indicatif})
*Total trouvé:* ${matches.length} membre(s)

${membersList}

👑 _Commande exécutée par un administrateur_
🤖 _Plugin tagpays - Anita-V4 by David Cyril Tech_`;

      // Envoyer le message avec mentions (compatible Anita-V4)
      ...
      // Log pour debug
      console.log(`[TAGPAYS] Admin ${m.sender} a tagué ${matches.length} membres de ${countryName}`);
